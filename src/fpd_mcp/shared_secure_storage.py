...
def resolve_api_key(
    ...
    secure_storage_getter: Callable[[], Optional[str]],
    env_var_name: str,
    default: Optional[str] = None,
) -> Optional[str]:
    """
    Resolve an API key using the standard 3-tier fallback chain: an
    explicit value takes priority, then unified secure storage, then an
    environment variable (with an optional default if unset).

    Consolidates the "param/kwarg or secure_storage.get() or
    os.getenv(...)" pattern duplicated across fpd_client.py's
    FPDClient.__init__/extract_with_mistral_ocr and config/settings.py's
    Settings.__init__.
    """
    if explicit_value:
