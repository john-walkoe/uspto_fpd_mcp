...
fpd:admin scope (the user-management tool). Rows are managed by the
admin-gated `FPD_manage_users` MCP tool or scripts/manage_mcp_users.py.
...
Backend is a single SQLite file (FPD_AUTH_DB_PATH, default
...
containers on ONE host (FPD mounts the shared paid-tier file PFW hosts); a network
...
class McpUserStore:
    ...
    async def get_refresh_any(self, token: str) -> dict[str, Any] | None:
        ...
        distinguish a replayed (spent) token from an unknown one (L3)."""
        async with self._db() as db:
            cur = await db.execute(
                "SELECT token_hash, client_id, email, scopes, expires_at, revoked "
    ...
    async def revoke_all_refresh_for(self, client_id: str, email: str) -> int:
        ...
        practical token family. Called on replay of a spent token (L3:
