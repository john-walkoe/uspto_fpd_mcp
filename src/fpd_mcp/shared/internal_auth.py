    ...
    def __init__(self, shared_secret: Optional[str] = None):
        ...
        if shared_secret is None:
            ...
            if not shared_secret:
                # Fall back to the DPAPI-protected shared secret (the same
                # file all 4 USPTO MCPs use). Without this, tokens are signed
                # with an ephemeral secret and PFW rejects every centralized
                # registration.
                try:
                    from ..shared_secure_storage import get_internal_auth_secret
                    shared_secret = get_internal_auth_secret()
                except Exception:
                    shared_secret = None
            if not shared_secret:
