...
class LogSanitizer:
    ...
    SENSITIVE_PATTERNS = [
        ...
        # Covers FPD's local /download/persistent/ and PFW's /document/persistent/.
        (r'(/(?:document|download)/persistent/)[A-Za-z0-9_\-]{16,}', r'\1[LINK_HASH]'),
        # Bare link hashes (sha256[:24] hex)
        (r'\b[a-f0-9]{24}\b', r'[LINK_HASH]'),

        # URL query strings — may embed tokens or search terms
        (r'(https?://[^\s"\'<>?]+)\?[^\s"\'<>]+', r'\1?[QUERY_REDACTED]'),

        # Long urlsafe-base64 tokens (secrets.token_urlsafe(32) is 43 chars)
        (r'\b(?=[A-Za-z0-9_\-]*[A-Za-z])(?=[A-Za-z0-9_\-]*[0-9])[A-Za-z0-9_\-]{40,}\b', r'[TOKEN]'),
...
class SanitizingFilter(logging.Filter):
    ...
    as httpx/uvicorn) happened to grab. The call-site logger wrappers stay
    for ergonomics; this filter is the guarantee.
    """
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if record.exc_text:
            record.exc_text = LogSanitizer.sanitize_string(
                record.exc_text, max_length=4000
            )
        return True
