...
tokens (S-06: one INTERNAL_AUTH_SECRET across all four USPTO MCPs, create-only,
with no rotation path)."""
...
from fpd_mcp.shared.internal_auth import InternalAuthToken
...
class TestMiddlewareGate:
    def test_split_secret_candidates_dedupes_strips_and_orders(self):
        from fpd_mcp.shared_secure_storage import split_secret_candidates
    ...
    def test_the_gate_accepts_current_and_previous(self):
        from fpd_mcp.middleware import _matches_any_candidate
    ...
    def test_the_gate_survives_a_non_ascii_presented_value(self):
        """L-1: compare_credential must not raise on a non-ASCII header."""
        from fpd_mcp.middleware import _matches_any_candidate

        assert _matches_any_candidate("café", ["current-value"]) is False


class TestSecretRotation:
    def test_a_comma_separated_secret_is_current_then_previous(self):
        token = InternalAuthToken(f"{_SECRET},{_PREVIOUS}").create_token("ptab-mcp")
...
class TestServiceTokenKeyDerivation:
    def test_hkdf_derivation_is_deterministic_and_root_specific(self):
        from fpd_mcp.shared.internal_auth import _derive_service_token_key
    ...
    def test_key_id_round_trips_on_a_minted_token(self):
        from fpd_mcp.shared.internal_auth import _service_token_key_id

        token = InternalAuthToken(_SECRET).create_token("ptab-mcp")
    ...
    def test_a_token_verifies_under_a_previous_root_by_key_derivation(self):
        ...
        token = InternalAuthToken(_PREVIOUS).create_token("ptab-mcp")
