"""Registration gate for FPD_manage_users (PFW/PTAB parity).

The tool must be absent from the registered tool set unless
FPD_ENABLE_USER_MANAGEMENT=true (default off — stdio never needs it, and
...
_PROBE = (
    ...
    "print('PRESENT' if 'FPD_manage_users' in names else 'ABSENT')\n"
...
def _probe(extra_env: dict) -> tuple[str, int]:
    env = {**os.environ}
    env.pop("FPD_ENABLE_USER_MANAGEMENT", None)
...
def test_manage_users_registered_when_enabled():
    present, count = _probe({"FPD_ENABLE_USER_MANAGEMENT": "true"})
...
def test_manage_users_refuses_http_without_oauth():
    ...
    env = dict(os.environ)
    env.setdefault("USPTO_API_KEY", "x" * 30)
    env["FPD_ENABLE_USER_MANAGEMENT"] = "true"
    ...
    assert "FPD_ENABLE_USER_MANAGEMENT=true requires" in result.stderr
