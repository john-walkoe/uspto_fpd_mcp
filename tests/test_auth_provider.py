...
from fpd_mcp.auth.provider import (  # noqa: E402
    _TXN_COOKIE,
    SCOPE_ADMIN,
    SCOPE_USER,
    FpdAuthProvider,
...
from fpd_mcp.auth.store import McpUserStore  # noqa: E402
from fpd_mcp.auth.settings import AuthSettings  # noqa: E402
...
class FakeUserStore:
    ...
    async def revoke_all_refresh_for(self, client_id: str, email: str) -> int:
        ...
        for row in self.refresh.values():
            if row["client_id"] == client_id and row["email"] == email and not row["revoked"]:
                row["revoked"] = True
                count += 1
        return count
...
) -> tuple[FpdAuthProvider, FakeUserStore]:
    store = store or FakeUserStore()
    provider = FpdAuthProvider(make_settings(**overrides), store)  # type: ignore[arg-type]
...
def test_build_auth_provider_rejects_http_base_url() -> None:
    with pytest.raises(ValueError, match="https"):
        build_auth_provider(
            make_settings(auth_base_url="http://mrbob.example.com"),
...
def test_email_from_claims_microsoft_upn_fallback() -> None:
    f = FpdAuthProvider._email_from_claims
...
async def test_load_access_token_rejects_garbage_and_internal() -> None:
    ...
    # M1: the plain internal token grants fpd:user ONLY — admin requires the
    # separate FPD_AUTH_INTERNAL_ADMIN_TOKEN (see below).
...
async def test_internal_admin_token_grants_admin_scope() -> None:
    """M1: a separate internal admin token grants fpd:user + fpd:admin;
    the plain internal token still only grants fpd:user."""
...
@pytest.mark.asyncio
async def test_refresh_replay_revokes_family(monkeypatch: pytest.MonkeyPatch) -> None:
    """L3: presenting an already-rotated (spent) refresh token revokes every
    other live token for that (client_id, email) — treat reuse as an
    incident, not a silent miss."""
    provider, store = make_provider()
    client = make_client()
    store.users["jane@firm.com"] = {
        "email": "jane@firm.com", "role": "user", "active": True,
    ...
    await store.put_refresh(
        "refresh-2", client_id=client.client_id, email="jane@firm.com",
        scopes=[SCOPE_USER], ttl_seconds=3600,
    )
    # Rotate refresh-1 (simulating a prior, legitimate exchange).
    await store.revoke_refresh("refresh-1")
    assert store.refresh["refresh-2"]["revoked"] is False

    logged = []
    monkeypatch.setattr(
        "fpd_mcp.auth.provider.security_logger.log_suspicious_activity",
        lambda **kwargs: logged.append(kwargs),
    )

    # Replay of the spent token -> None, and the WHOLE family is revoked.
    assert await provider.load_refresh_token(client, "refresh-1") is None
    assert store.refresh["refresh-2"]["revoked"] is True
    assert logged and logged[0]["activity_description"] == "refresh_token_replay"

    # An unknown token (never issued) is just a miss — no family revocation,
    # no security log entry.
    logged.clear()
    assert await provider.load_refresh_token(client, "never-issued") is None
    assert not logged


@pytest.mark.asyncio
async def test_oauth_rate_limiter_blocks_after_threshold() -> None:
    """L5: the OAuth callback route enforces a per-IP fixed window."""
    provider, store = make_provider()
    store.users["jane@firm.com"] = {
        "email": "jane@firm.com", "role": "user", "active": True,
        "display_name": None, "last_login_idp": None,
    ...
    async def fake_exchange(idp_: str, code: str, nonce: str):
        return {"email": "jane@firm.com", "email_verified": True}
    ...
    async def one_request():
        ...
        txn = parse_qs(urlparse(url).query)["txn"][0]
        scope = {
            "type": "http", "method": "GET", "path": "/auth/callback/google",
            "query_string": f"state={txn}&code=up".encode(),
            "headers": [], "path_params": {"idp": "google"},
            "client": (ip, 12345),
        }
        req = Request(scope)
        return req
    ...
    for _ in range(30):
        req = await one_request()
        provider._exchange_and_verify = fake_exchange  # type: ignore[method-assign]
        resp = await provider._callback_endpoint(req)
        assert resp.status_code != 429

    # 31st request from the same IP within the window is rate-limited.
    req = await one_request()
    resp = await provider._callback_endpoint(req)
    assert resp.status_code == 429
...
class TestMcpUserStore:
    ...
    async def test_refresh_lifecycle(self, store: McpUserStore) -> None:
        await store.put_refresh(
            ...
            scopes=["fpd:user"], ttl_seconds=3600,
        )
        row = await store.get_refresh("tok")
        assert row is not None
        assert row["scopes"] == ["fpd:user"]
...
async def test_admin_tools_scope_gated() -> None:
    ...
    from fpd_mcp.main import _attach_admin_scope_checks

    mcp = FastMCP("gating-test")

    @mcp.tool(name="FPD_manage_users")
    def fpd_manage_users() -> str:  # same name as the real admin tool
        return "ok"

    @mcp.tool(name="FPD_Search_petitions_minimal")
    def Search_petitions_minimal() -> str:
        return "ok"
    ...
    assert tools["FPD_manage_users"].auth
    assert not tools["FPD_Search_petitions_minimal"].auth

    def token(scopes: list[str]) -> AccessToken:
        return AccessToken(token="t", client_id="c", scopes=scopes)

    gate = tools["FPD_manage_users"].auth
    ctx_admin = AuthContext(
        token=token([SCOPE_USER, SCOPE_ADMIN]),
        component=tools["FPD_manage_users"],
    )
    ctx_user = AuthContext(
        token=token([SCOPE_USER]), component=tools["FPD_manage_users"]
    )
    ctx_anon = AuthContext(token=None, component=tools["FPD_manage_users"])
...
async def test_oauth_routes_mounted_on_http_app() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        assert "fpd:user" in doc.get("scopes_supported", [])
...
async def test_mode_none_stack_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import os
    os.environ.setdefault("USPTO_API_KEY", "x" * 30)
    import fpd_mcp.shared_secure_storage as sss
    from fpd_mcp.main import (
...
async def test_allowed_redirect_hosts_can_be_extended(
    ...
    monkeypatch.setenv("FPD_AUTH_ALLOWED_REDIRECT_HOSTS", "partner.example")
    provider, _ = make_provider()
    client = OAuthClientInformationFull(
...
async def test_open_registration_opt_in_restores_the_old_behavior(
    ...
    monkeypatch.setenv("FPD_AUTH_OPEN_REGISTRATION", "true")
    provider, _ = make_provider()
    client = OAuthClientInformationFull(
