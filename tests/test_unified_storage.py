...
# DESTRUCTIVE: this test overwrites the REAL DPAPI-stored keys for FPD *and*
# the other 3 USPTO MCPs (PFW/PTAB/Enriched Citations) mid-test. Opt in
# deliberately with FPD_RUN_KEY_TESTS=1.
if not os.getenv("FPD_RUN_KEY_TESTS"):
    pytest.skip(
        "Destructive unified-storage test skipped (overwrites real stored keys "
        "across all USPTO MCPs). Set FPD_RUN_KEY_TESTS=1 to run deliberately.",
