...
Two tools are eager (`FPD_Search_petitions_minimal`, `FPD_get_guidance`); the
rest are deferred, and that split is the server's context saving.
`defer_loading` is not an MCP spec field, and MCP SDK 2.x sieves non-spec
fields out of spec-method results per protocol version — see `fastmcp_compat`
for the mechanism. Without that shim every tool arrives eagerly loaded, with no
...
from fpd_mcp.fastmcp_compat import apply as apply_compat

# main.py applies the shim at import; call it directly so this module also
# passes when run alone.
apply_compat()

# The eager set as served by FastMCP 3.4.0 immediately before the FastMCP 4
# migration, captured from a real stdio tools/list. Pinned here so the upgrade
# cannot quietly change which tools load eagerly. Matches the two tools
# SERVER_INSTRUCTIONS lists as ALWAYS-AVAILABLE.
EAGER = {
...
def test_tool_titles_are_pinned_to_tool_names():
    ...
    ("FPD Get Guidance"); `_pin_tool_titles` keeps the displayed label equal to
    the name, as every doc and guidance string spells it.
    """
    from fastmcp.tools.base import Tool
