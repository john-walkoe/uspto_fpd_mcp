# Obtaining USPTO FPD MCP Server API Keys
...
The USPTO FPD MCP server (and all of the author's USPTO MCP servers) only use Mistral OCR to have the LLM read **publicly available USPTO documents**. This usage should not raise information governance concerns.

However, if you use the API key separately outside the scope of the USPTO FPD MCP to:
...
12. Fill out the key creation form:
    - **Key name**: Descriptive name (e.g., "USPTO FPD MCP OCR")
...
16. When running the deployment script, paste the API key when prompted:
    ...
    ```
    - You can press Enter to skip if you want to set this up later via the `C:\Users\[USERNAME]\uspto_fpd_mcp\deploy\manage_api_keys.ps1` script
...
```powershell
C:\Users\[USERNAME]\uspto_fpd_mcp\deploy\manage_api_keys.ps1
```
